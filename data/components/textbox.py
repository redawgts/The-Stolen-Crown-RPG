...
class DialogueBox:
    ...
    def make_dialogue_box_image(self):
        ...
        dialogue_image = self.font.render(self.dialogue_list[self.index], True, c.NEAR_BLACK)
...
class TextHandler:
        ...
        if self.textbox:
            ...
            if self.textbox.done:
                ...
                if self.textbox.index < (len(self.textbox.dialogue_list) - 1):
                    ...
                    self.talking_sprite.name == "oldmanbrother"
                    and self.game_data["talked to sick brother"]
                    and not self.game_data["has brother elixir"]
                ):
                    ...
                    if (
                        self.game_data["has brother elixir"]
                        and not self.game_data["elixir received"]
                    ):
    ...
    def add_new_item_to_inventory(self, item):
        ...
        if item in potions:
            inventory[item] = dict([("quantity", 1), ("value", 15)])
        elif item == "ELIXIR":
            inventory[item] = dict([("quantity", 1)])
        elif item == "Fire Blast":
            inventory[item] = dict([("magic points", 40), ("power", 15)])
        else:
            pass
...
class ItemBox:
    def __init__(self, dialogue, item):
